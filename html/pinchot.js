function pinchot(color, type) {
  ...
  switch ( type ) {
    // Apple touch icon rond en 120x120
    case 'round':
    var html = '<svg'+
...
jQuery(document).ready(function() {
  // fonction qui transforme le SVG en data:image/png;base64 à travers le canvas
  function svg2png(id) {
          ...
          for (var i in size) {
            ...
            jQuery('#pinchots').append(
              '<canvas id="canvas_'+size[i]+k+'" '+
                'download="'+size[i].replace('round','').replace('fav','fav_')+k+'.png" '+ // nom de téléchargement
            ...
            // on passe le SVG dans le canvas pour en ressortir le data:image/png;base64
            svg2png(size[i]+k);

          }
        }
      }
      // on ajoute un lien pour télécharger les icônes avec le nom de fichier qui va bien (sinon c'est clic-droit + renommer "canvas.png" à chaque fois)
      jQuery('canvas').wrap(function() {
        canvas = document.getElementById(jQuery(this).attr('id')); // -_-
        return '<a href="'+canvas.toDataURL('image/png')+'" download="'+jQuery(this).attr('download')+'"></a>';
      });
    }
  );
});
